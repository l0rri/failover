# Yelp_review.json parser (Milestone 1, task 1c)
# CS 451-02 Spring 2018
# Team "The IT Crowd" (Geilenfeldt, Rink, Williams)
[CmdletBinding()]
param (
    # The path (absolute or relative) to the pre-provided Yelp_review.JSON file.
    # This will default to looking inside a folder named "Yelp-CptS451-2018" stored alongside the script itself.
    [ValidateScript({
        If (-not (Test-Path $_)) {
            throw [ArgumentException]::new("Specified path does not exist.", "ReviewJSONPath")
        } elseif ((Get-Item $_).Extension -ne ".json") {
            throw [ArgumentException]::new("Specified file does not appear to be a JSON document.", "ReviewJSONPath")
        }
    ...
    [Parameter(
        ...
        Position = 0,
        HelpMessage = "Provide the path to the pre-generated Yelp_review.json file. Relative and absolute paths are supported."
    )]
    [String]$ReviewJSONPath = (Join-Path $PSScriptRoot ".\Yelp-CptS451-2018\Yelp_review.JSON"),
...
process {
    ...
    Write-Verbose "Loading JSON from `"$($ReviewJSONPath | Split-Path -Leaf)`"..."

    # Retrieve JSON, rehydrate to objects, then iterate.
    # The review JSON appears to be a bunch of smaller JSON documents concatenated together with linebreaks as delimiters.
    ...
    Get-Content $ReviewJSONPath | ForEach-Object {$_ |  ConvertFrom-Json} | ForEach-Object -Process {

        Write-Verbose "Processing review with ID `"$($_.Review_ID)`"."

        # We don't have to do anything for this file, actually. (No attributes to overwrite/remove/aggregate, etc.)
        Return $_
    
    } -End {Write-Verbose "Review data processing complete."} |
